...
<div class="d-flex align-items-center">
    <button class="btn-add mb-4 border-0 px-3 py-2 text-black rounded-3 fw-medium d-flex align-items-center gap-2"
        onclick="location.href='{{route('kepalagudang-entry')}}'">
    ...
    <button class="btn-add mb-4 border-0 px-3 py-2 text-black rounded-3 fw-medium d-flex align-items-center gap-2 ms-2"
        onclick="location.href='{{route('pdfkepalagudang')}}'">
...
<table class="table table-bordered text-center">
    ...
    @foreach($data as $kg)
        <tr>
            ...
            <td>
                <button type="button" class="btn btn-dark" data-bs-toggle="modal" data-bs-target="#target{{ $kg->Id_KepalaGudang }}">Edit</button>
                <button type="button" onclick="window.location='{{ route('kepalagudang-destroy', $kg->Id_KepalaGudang) }}'" class="btn btn-dark">Delete</button>
            </td>
            <div class="modal fade" id="target{{$kg->Id_KepalaGudang }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h1 class="modal-title fs-5" id="exampleModalLabel">Form Edit Data kepala Gudang</h1>
                      <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form action="{{ route('kepalagudang-update', ['id' => $kg->Id_KepalaGudang]) }}" method="post" enctype="multipart/form-data"> 
                            @csrf
                            <div class="mb-4">
                                <label for="id-kepalagudang" class="form-label">ID Kepala Gudang</label>
                                <input type="text" class="form-control input" id="id-kepalagudang" name="id_kepalagudang" placeholder="Masukkan ID Kepala Gudang" value="{{$kg->Id_KepalaGudang}}">
                            </div>
                            <div class="mb-4">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" class="form-control input" id="email" name="eml" placeholder="Masukkan Email" value="{{$kg->Email}}">
                            </div>
                            <div class="mb-4">
                                <label for="password" class="form-label">Password</label>
                                <input type="password" class="form-control input" id="password" name="password" placeholder="Masukkan Password" value="{{substr($kg->Password, 0, 10)}}">
                            </div>
