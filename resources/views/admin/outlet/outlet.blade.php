...
<div class="d-flex align-items-center">
    <button class="btn-add mb-4 border-0 px-3 py-2 text-black rounded-3 fw-medium d-flex align-items-center gap-2"
        onclick="location.href='{{route('outlet-entry')}}'">
    ...
    <button class="btn-add mb-4 border-0 px-3 py-2 text-black rounded-3 fw-medium d-flex align-items-center gap-2 ms-2"
        onclick="location.href='{{route('pdfoutlet')}}'">
...
<table class="table table-bordered text-center">
    ...
    @foreach($outlet as $ot)
        <tr>
            ...
            <td>{{$ot->No_Telp}}</td>
            <td>
                <button type="button" class="btn btn-dark" data-bs-toggle="modal" data-bs-target="#target{{ $ot->Id_Outlet }}">Edit</button>
                <button type="button" onclick="window.location='{{ route('outlet-destroy', $ot->Id_Outlet) }}'" class="btn btn-dark">Delete</button>
            </td>
        </tr>
        <div class="modal fade" id="target{{ $ot->Id_Outlet }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
              <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('outlet-update', ['id' => $ot->Id_Outlet]) }}" method="post">
                        @csrf
                        <div class="mb-4">
                            <label for="id-outlet" class="form-label">ID Outlet</label>
                            <input type="text" class="form-control input" id="id-outlet" name="id_outlet" placeholder="Masukkan ID Outlet" value="{{$ot->Id_Outlet}}">
                        </div>
                        <div class="mb-4">
                            <label for="nama-outlet" class="form-label">Nama Outlet </label>
                            <input type="text" class="form-control input" id="nama-outlet" name="nama_outlet" placeholder="Masukkan Nama Outlet" value="{{$ot->Nama_Outlet}}">
                        </div>
                        <div class="mb-4">
                            <label for="alamat" class="form-label">Alamat</label>
                            <input class="form-control input" id="alamat" rows="5" name="alt" placeholder="Masukkan Alamat" value="{{$ot->Alamat}}">
                        </div>
                        <div class="mb-4">
                            <label for="no-telp" class="form-label">No.Telp</label>
                            <input type="text" class="form-control input" id="no-telp" name="no_telp" placeholder="Masukkan NO.Telp" value="{{$ot->No_Telp}}">
                        </div>
