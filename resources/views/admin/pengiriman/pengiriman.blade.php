...
<div class="d-flex align-items-center">
    <button class="btn-add mb-4 border-0 px-3 py-2 text-black rounded-3 fw-medium d-flex align-items-center gap-2"
        onclick="location.href='{{route('pengiriman-entry')}}'">
    ...
    <button class="btn-add mb-4 border-0 px-3 py-2 text-black rounded-3 fw-medium d-flex align-items-center gap-2 ms-2"
        onclick="location.href='{{route('pdfpengiriman')}}'">
...
<table class="table table-bordered text-center">
    ...
    @foreach($pengiriman as $pr)
        <tr>
            ...
            <td>
                <button type="button" class="btn btn-dark" data-bs-toggle="modal" data-bs-target="#target{{ $pr->Id_Pengiriman }}">Edit</button>
                <button type="button" onclick="window.location='{{ route('pengiriman-destroy', $pr->Id_Pengiriman) }}'" class="btn btn-dark">Delete</button>
            </td>
            <div class="modal fade" id="target{{ $pr->Id_Pengiriman }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h1 class="modal-title fs-5" id="exampleModalLabel">Form Edit Data Pengiriman</h1>
                      <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form action="{{ route('pengiriman-update', ['id' => $pr->Id_Pengiriman]) }}" method="post" enctype="multipart/form-data"> 
                            @csrf
                            <div class="mb-4">
                                <label for="id-pengiriman" class="form-label">ID Pengiriman</label>
                                <input type="text" class="form-control input" id="id-pengiriman" name="id_pengiriman" placeholder="Masukkan ID Pengiriman" value="{{ $pr->Id_Pengiriman }}">
                            </div>
                            <div class="mb-4">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="date" class="form-control input" id="tgl-pengiriman" name="tanggal_pengiriman" placeholder="Masukkan Tanggal Pengiriman" value="{{$pr->Tanggal_Pengiriman}}">
                            </div>
