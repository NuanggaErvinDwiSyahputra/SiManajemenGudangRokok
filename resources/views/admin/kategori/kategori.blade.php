...
<div class="d-flex align-items-center">
    <button class="btn-add mb-4 border-0 px-3 py-2 text-black rounded-3 fw-medium d-flex align-items-center gap-2"
        onclick="location.href='{{route('kategori-entry')}}'">
    ...
    <button class="btn-add mb-4 border-0 px-3 py-2 text-black rounded-3 fw-medium d-flex align-items-center gap-2 ms-2"
        onclick="location.href='{{route('pdfkategori')}}'">
...
<table class="table table-bordered text-center">
    ...
    @foreach($kategori as $kg)
        <tr>
            ...
            <td>{{$kg->Kategori}}</td>
            <td>
                <button type="button" class="btn btn-dark" data-bs-toggle="modal" data-bs-target="#target{{ $kg->Id_Kategori }}">Edit</button>
                <button type="button" onclick="window.location='{{ route('kategori-destroy', $kg->Id_Kategori) }}'" class="btn btn-dark">Delete</button>
            </td>
        </tr>

        {{-- Modal Edit --}}
        <div class="modal fade" id="target{{ $kg->Id_Kategori }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h1 class="modal-title fs-5" id="exampleModalLabel">Form Edit Data Kategori</h1>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('kategori-update', ['id' => $kg->Id_Kategori]) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-4">
                            <label for="id-kategori" class="form-label">ID Kategori</label>
                            <input type="text" class="form-control input" id="id-kategori" name="id_kategori" placeholder="Masukkan ID Kategori" value="{{$kg->Id_Kategori}}">
                        </div>
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            ...
                            <select class="form-control input" id="kategori" name="kategori" >
                                ...
                                <option value="Kretek Original" {{ $kg->Kategori == 'Kretek Original' ? 'selected' : '' }}>Kretek Original
                                <option value="Kretek Flavour" {{ $kg->Kategori == 'Kretek Flavour' ? 'selected' : '' }}>Kretek Flavour
                                <option value="Putihan" {{ $kg->Kategori == 'Putihan' ? 'selected' : '' }}>Putihan
                            </select>
                        </div>
