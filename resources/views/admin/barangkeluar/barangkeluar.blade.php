...
<div class="d-flex align-items-center">
    <button class="btn-add mb-4 border-0 px-3 py-2 text-black rounded-3 fw-medium d-flex align-items-center gap-2"
        onclick="location.href='{{route('barangkeluar-entry')}}'">
    ...
    <button class="btn-add mb-4 border-0 px-3 py-2 text-black rounded-3 fw-medium d-flex align-items-center gap-2 ms-2"
        onclick="location.href='{{route('pdfbarangkeluar')}}'">
...
<table class="table table-bordered text-center">
    ...
    @foreach($barangkeluar as $bk)
        <tr>
            <td>{{$bk->Id_BarangKeluar}}</td>
            <td>{{$bk->Id_Penjualan}}</td>
            <td>{{$bk->Id_Admin}}</td>
            <td>{{$bk->Tanggal_Keluar}}</td>
            <td>{{$bk->Jumlah}}</td>
            <td>
                <button type="button" class="btn btn-dark" data-bs-toggle="modal" data-bs-target="#target{{ $bk->Id_BarangKeluar }}">Edit</button>
                <button type="button" onclick="window.location='{{ route('barangkeluar-destroy', $bk->Id_BarangKeluar) }}'" class="btn btn-dark">Delete</button>
            </td>
        </tr>
        <div class="modal fade" id="target{{ $bk->Id_BarangKeluar }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h1 class="modal-title fs-5" id="exampleModalLabel">Form Edit Data Barang Keluar</h1>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('barangkeluar-update', ['id' => $bk->Id_BarangKeluar]) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-4">
                            <label for="id-barangkeluar" class="form-label">ID Barang Keluar</label>
                            <input type="text" class="form-control input" id="id-barangkeluar" name="id_barangkeluar"
                                placeholder="Masukkan ID Barang Keluar" value='{{$bk->Id_BarangKeluar}}'>
                        </div>
                        <div class="mb-4">
