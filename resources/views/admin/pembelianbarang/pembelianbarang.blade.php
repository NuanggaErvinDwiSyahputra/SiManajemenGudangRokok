...
<div class="d-flex align-items-center">
    <button class="btn-add mb-4 border-0 px-3 py-2 text-black rounded-3 fw-medium d-flex align-items-center gap-2"
        onclick="location.href='{{route('pembelianbarang-entry')}}'">
    ...
    <button class="btn-add mb-4 border-0 px-3 py-2 text-black rounded-3 fw-medium d-flex align-items-center gap-2 ms-2"
        onclick="location.href='{{route('pdfpembelianbarang')}}'">
    ...
    @foreach($pembelian as $pb)
        <tr>
            ...
            <td>
                ...
                <button type="button" class="btn btn-dark" data-bs-toggle="modal" data-bs-target="#target{{ $pb->Id_Pembelian }}">Edit</button>
                <button type="button" onclick="window.location='{{ route('pembelianbarang-destroy', $pb->Id_Pembelian) }}'" class="btn btn-dark ms-2">Delete</button>
                </div>
            </td>
            <div class="modal fade" id="target{{ $pb->Id_Pembelian }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h1 class="modal-title fs-5" id="exampleModalLabel">Form Edit Data Pembelian Barang</h1>
                      <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form action="{{ route('pembelianbarang-update', ['id' => $pb->Id_Pembelian]) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="mb-4">
                                <label for="id-pembelian" class="form-label">ID Pembelian </label>
                                <input type="text" class="form-control input" id="id-pembelian" name="id_pembelian" placeholder="Masukkan ID Pembelian" value="{{$pb->Id_Pembelian}}">
                            </div>
                            <div class="mb-4">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" class="form-control input" id="nama-barang" name="nama_barang" placeholder="Masukkan Nama Barang" value="{{$pb->Nama_Barang}}">
                            </div>
                            <div class="mb-4">
                                <label for="jumlah-pembelian" class="form-label">Jumlah Pembelian</label>
                                <input type="number" class="form-control input" id="jumlah-pembelian" name="jumlah_pembelian" placeholder="Masukkan Jumlah Pembelian" value="{{$pb->Jumlah}}">
                            </div>
                            <div class="mb-4">
                                <label for="tanggal-pembelian" class="form-label">Tanggal Pembelian</label>
                                <input type="date" class="form-control input" id="tanggal-pembelian" name="tanggal_pembelian" placeholder="Masukkan Tanggal Pembelian" value="{{$pb->Tanggal_Pembelian}}">
                            </div>
                             <div class="mb-4">
                                <label for="harga-pembelian" class="form-label">Harga Pembelian</label>
                                <input type="text" class="form-control input" id="harga-pembelian" name="harga_pembelian" placeholder="Masukkan Harga Pembelian" value="{{$pb->Harga_Pembelian}}">
                            </div>
