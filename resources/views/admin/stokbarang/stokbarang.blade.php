...
<div class="d-flex align-items-center">
    <button class="btn-add mb-4 border-0 px-3 py-2 text-black rounded-3 fw-medium d-flex align-items-center gap-2"
        onclick="location.href='{{route('stokbarang-entry')}}'">
    ...
    <button class="btn-add mb-4 border-0 px-3 py-2 text-black rounded-3 fw-medium d-flex align-items-center gap-2 ms-2"
        onclick="location.href='{{route('pdfstokbarang')}}'">
...
<table class="table table-bordered text-center">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <button type="button" class="btn btn-dark" data-bs-toggle="modal" data-bs-target="#target{{ $sd->Id_StokBarang }}">Edit</button>
                <button type="button" onclick="window.location='{{ route('stokbarang-destroy', $sd->Id_StokBarang) }}'" class="btn btn-dark">Delete</button>
            </td>
        </tr>
        <div class="modal fade" id="target{{ $sd->Id_StokBarang }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h1 class="modal-title fs-5" id="exampleModalLabel">Form Edit Data Stok Barang</h1>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('stokbarang-update', ['id' => $sd->Id_StokBarang]) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-4">
                            <label for="id-stokbarang" class="form-label">ID Stok Barang</label>
                            <input type="text" class="form-control input" id="id-stokbarang" name="id_stokbarang" placeholder="Masukkan ID Stok Barang" value="{{$sd->Id_StokBarang}}">
                        </div>
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" class="form-control input" id="stokbarang" name="stok_barang" placeholder="Masukkan Stok Barang" value="{{$sd->Stok_Barang}}">
                        </div>
