...
<div class="d-flex align-items-center">
    <button class="btn-add mb-4 border-0 px-3 py-2 text-black rounded-3 fw-medium d-flex align-items-center gap-2"
        onclick="location.href='{{route('barangreturnpembelian-entry')}}'">
    ...
    <button class="btn-add mb-4 border-0 px-3 py-2 text-black rounded-3 fw-medium d-flex align-items-center gap-2 ms-2"
        onclick="location.href='{{route('pdfbarangreturnpembelian')}}'">
...
<table class="table table-bordered text-center">
    ...
    <tbody>
    @foreach($barangreturnpembelian as $brp)
        <tr>
            ...
            <td>
                ...
                <button type="button" class="btn btn-dark" data-bs-toggle="modal" data-bs-target="#target{{ $brp->Id_BarangReturnPembelian }}">Edit</button>
                <button type="button" onclick="window.location='{{ route('barangreturnpembelian-destroy', $brp->Id_BarangReturnPembelian) }}'" class="btn btn-dark ms-2">Delete</button>
                </div>
            </td>
        </tr>
        <div class="modal fade" id="target{{ $brp->Id_BarangReturnPembelian }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h1 class="modal-title fs-5" id="exampleModalLabel">Form Edit Data Barang Return Pembelian</h1>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('barangreturnpembelian-update', ['id' => $brp->Id_BarangReturnPembelian]) }}" method="post">
                        @csrf
                        <div class="mb-4">
                            <label for="id-barangreturnpembelian" class="form-label">ID Barang Return Pembelian</label>
                            <input type="text" class="form-control input" id="id-barangreturnpembelian" name="id_barangreturnpembelian" placeholder="Masukkan ID Barang Return Pembelian" value="{{$brp->Id_BarangReturnPembelian}}">
                        </div>
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" class="form-control input" id="tgl-returnpembelian" name="tgl_returnpembelian" placeholder="Masukkan Tanggal Return Pembelian" value="{{$brp->Tanggal_Return}}">
                        </div>
                        <div class="mb-4">
                            <label for="jml-returnpembelian" class="form-label">Jumlah Barang Return Pembelian</label>
                            <input type="number" class="form-control input" id="jml-returnpembelian" name="jml_returnpembelian" placeholder="Masukkan Barang Return Pembelian" value="{{$brp->Jumlah_BarangReturnPembelian}}">
                        </div>
